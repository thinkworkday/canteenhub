...
import { addStoreUser } from '@store/actions/vendor.actions';

const SidebarNewUsers = ({ parent, open, toggleSidebar }) => {
  // ** States
  ...
  const onSubmit = async (values) => {
    ...
    if (isObjEmpty(errors)) {
      ...
      values.role = 'store';
      values.parentVendor = parent;

      try {
        await dispatch(addStoreUser(values));
        toast.success(
          <>
  ...
  return (
    ...
    >
      <Form onSubmit={handleSubmit(onSubmit)} className="h-100 d-flex flex-column ">
        ...
        <small>Note: Creating a new store user will send an invite to the specified address with login instructions</small>
