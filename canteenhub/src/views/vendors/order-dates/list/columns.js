  ...
  return [
    {
      ...
      sortable: true,
      minWidth: '250px',
      cell: (row) => <Link onClick={() => store.dispatch(fetchEvent(row._id))} to={`/${loggedUser.role}/order-dates/edit/${row._id}`}>{`${getDeliveryDate(row.date, row.deliveryTime)}`}</Link>,
    },
    {
      name: 'Cutoff',
      selector: (row) => getCutOffDate(row.date, row.cutoffPeriod, true),
      sortable: true,
    },
    {
      name: 'Event Title',
    ...
    {
      name: 'Group',
      selector: (row) => row.group?.companyName,
      sortable: true,
    },
    {
      name: 'Menu',
      selector: (row) => (row.menus[0] ? row.menus[0].name : <span className="text-muted">{row.menuDefault[0].name}</span>),
      sortable: true,
    },
    {
      name: 'Status',
          ...
          <>
            <UncontrolledDropdown>
              ...
              <DropdownMenu right>
                <DropdownItem
                  tag={Link}
                  to={`/vendor/order-dates/edit/${row._id}`}
                  className="w-100"
