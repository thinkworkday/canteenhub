...
import {
  Alert, Badge, Card, CardBody, FormGroup, Row, Col, Input, Form, Button, Label, Spinner, UncontrolledTooltip,
} from 'reactstrap';
...
import {
  getStore, updateStore, getStoreUsers, addBankAccount,
} from '@store/actions/vendor.actions';
...
const StoreForm = () => {
  ...
  const selectedStore = useSelector((state) => state.stores.selectedStore[0]);
  ...
  const onSubmit = async (data) => {
    ...
    if (isObjEmpty(errors)) {
      ...
      try {
        ...
        window.location = `${process.env.REACT_APP_SITE_URL}/admin/users/vendors`;
        // toggleSidebar();
        setProcessing(false);
      } catch (err) {
        ...
        <Col sm="12" lg="2">
          ...
          <Card>
            <CardBody>
              ...
              <Badge color={selectedStore?.stripeAccountStatus === 'active' ? 'success' : 'light-secondary'}>
                {selectedStore ? selectedStore.stripeAccountStatus : 'not started'}
              </Badge>
              <div className="mt-1">
                Account Id:
                {' '}
            ...
            {selectedStore?.stripeAccountStatus !== 'active' ? (
              <CardFooter>
                <Button.Ripple onClick={() => bankAccountHandler(selectedStore)} color="primary">
                  {selectedStore.stripeAccountStatus === 'in progress' ? 'Continue Application >' : 'Get Connected >' }
                </Button.Ripple>
              </CardFooter>
            ) : <></>}
