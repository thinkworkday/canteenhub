...
import { updateMarketProvide, getMarketProvide } from '@store/actions/market.actions';
...
const MarketProvideEdit = () => {
  // ** Vars
  const dispatch = useDispatch();
  const { id } = useParams();

  const selectedMarketProvide = useSelector((state) => state.marketProvides.selectedMarketProvide);
  ...
  const [provideLogo, setProvideLogo] = useState();
  ...
  useEffect(() => {
    if (selectedMarketProvide) {
      // set defaults
      const fields = ['title', 'content'];
      fields.forEach((field) => {
        setValue(field, selectedMarketProvide[field]);
      });

      // Set select items
      setProvideLogo(selectedMarketProvide.provideLogo);
    }
  }, [selectedMarketProvide, errors]);

  const onSubmit = async (data) => {
    setProcessing(true);

    try {
      await dispatch(updateMarketProvide(selectedMarketProvide._id, data));
                  ...
                  <Row>
                    <Col md="12" sm="12" lg="12">
                      ...
                      <FormGroup className="mb-4">
                        ...
                        {provideLogo && !showDropzone ? (
                          <div className="my-2">
                            <img src={provideLogo} className="img-fluid" style={{ maxHeight: '250px' }} alt="Content Logo" />
                            <div className="my-2">
...
export default MarketProvideEdit;
