...
import { updateMarketPartner, getMarketPartner } from '@store/actions/market.actions';
...
const MarketPartnerEdit = () => {
  // ** Vars
  const dispatch = useDispatch();
  const { id } = useParams();

  const selectedMarketPartner = useSelector((state) => state.marketPartners.selectedMarketPartner);
  ...
  useEffect(() => {
    if (selectedMarketPartner) {
      // set defaults
      const fields = ['title', 'link', 'content'];
      fields.forEach((field) => {
        setValue(field, selectedMarketPartner[field]);
      });

      // Set select items
      setpartnerLogo(selectedMarketPartner.partnerLogo);
    }
  }, [selectedMarketPartner]);

  const onSubmit = async (data) => {
    setProcessing(true);

    try {
      await dispatch(updateMarketPartner(selectedMarketPartner._id, data));
      ...
      window.location = `${process.env.REACT_APP_SITE_URL}/admin/market/partners`;
      setProcessing(false);
    } catch (err) {
                  ...
                  <Row>
                    <Col md="12" sm="12" lg="12">
                      ...
                      <FormGroup className="mb-4">
                        ...
                        {partnerLogo && !showDropzone ? (
                          <div className="my-2">
                            <img src={partnerLogo} className="img-fluid" style={{ maxHeight: '250px' }} alt="Content Logo" />
                            <div className="my-2">
                        ...
                        {Object.keys(errors).length && errors.partnerLogo ? (
...
export default MarketPartnerEdit;
